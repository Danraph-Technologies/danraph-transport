...
const Settings = () => {
  ...
  // Your existing useEffect for phone country detection is fine.
  useEffect(() => {
  ...
  const handleLogout = async () => {
    ...
    await logout();
    // Redirect the user to the login page after they are logged out
    // The 'replace: true' option prevents the user from clicking the "Back" button
    // and returning to the protected settings page.
    navigate("/login", { replace: true });
  };
  return (
    <div className="sm:px-5  pb-12 items-center flex justify-center sm:justify-start">
      <div className="max-w-[767px] w-full px-4 py-3 pt-5 pb-12 border rounded-lg border-gray-300">
        ...
        <div className="flex items-center gap-7 py-5 ">
          <img
            src={img1}
            alt=""
            className="sm:max-w-[95px]  w-full sm:max-h-[95px] max-w-[80px] max-h-[80px] h-full rounded-full"
          />
          <label
            htmlFor="file-upload"
        ...
        <form action="" className="flex flex-col gap-5">
          ...
          <button
            type="button" // CRITICAL: Change type to "button" to prevent form submission
